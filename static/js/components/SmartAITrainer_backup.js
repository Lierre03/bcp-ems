...
window.SmartAITrainer = function SmartAITrainer() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '', timelines: { day1: [] }, budgetCategories: [], additionalResources: [],
    startDate: '', endDate: ''
  });
  ...
  const getNumberOfDays = () => {
    ...
    return Math.max(1, Math.min(diff, 10)); // Max 10 days
  };
  ...
  const venueOptions = ['Auditorium', 'Gymnasium', 'Main Hall', 'Cafeteria', 'Lab', 'Courtyard', 'Library', 'Covered Court'];
  ...
  // Timeline helpers - now support multiple days
  const addTimelinePhase = (dayKey = 'day1') => {
    setFormData(prev => {
      const dayTimeline = prev.timelines[dayKey] || [];
      const lastPhase = dayTimeline[dayTimeline.length - 1];
      const startTime = lastPhase ? lastPhase.endTime : '09:00';
      const endTime = lastPhase ? lastPhase.endTime : '10:00';

      return {
        ...prev,
        timelines: {
          ...prev.timelines,
          [dayKey]: [...dayTimeline, { startTime, endTime, phase: '' }]
        }
      };
    });
  };

  const updateTimelinePhase = (index, field, value, dayKey = 'day1') => {
    setFormData(prev => {
      const dayTimeline = [...(prev.timelines[dayKey] || [])];
      dayTimeline[index][field] = value;
      return {
        ...prev,
        timelines: { ...prev.timelines, [dayKey]: dayTimeline }
      };
    });
  };

  const removeTimelinePhase = (index, dayKey = 'day1') => {
    setFormData(prev => ({
      ...prev,
      timelines: {
        ...prev.timelines,
        [dayKey]: (prev.timelines[dayKey] || []).filter((_, i) => i !== index)
      }
    }));
  };
  ...
  const saveAndTrain = async () => {
    if (!formData.name || !formData.budget) {
        alert("Please fill out Event Name and Budget.");
        return;
    }

    setIsProcessing(true);
    try {
      // Step 1: Save training data
      const saveResponse = await fetch('/api/ml/add-training-data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          ...
          // Send activities: single day = array, multi-day = object {day1: [], day2: [], ...}
          activities: numberOfDays === 1 ? formData.timelines.day1 : formData.timelines,
      ...
      // Step 2: Train models
      const trainResponse = await fetch('/api/ml/train-models', { method: 'POST', credentials: 'include' });
      const trainData = await trainResponse.json();

      // Show result
      if (trainData.success) {
        alert(`✅ Training complete!\n\nModels trained on ${trainData.metrics ? `${trainData.message}\nBudget R²: ${trainData.metrics.budget_r2}\nEquipment Accuracy: ${trainData.metrics.equipment_acc}` : trainData.message}`);
      } else {
        alert(`✅ Data saved!\n\n${trainData.message || 'Add more events to train models.'}`);
      }
  ...
  return (
    <div className="flex flex-col gap-4">
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
            ...
            {activeFormTab === 'basic' && (
              <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" value={formData.startDate} min={new Date().toISOString().split('T')[0]} onChange={(e) => setFormData({...formData, startDate: e.target.value})} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm" />
                </div>
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">End Date</label>
                    <input type="date" value={formData.endDate} min={formData.startDate || new Date().toISOString().split('T')[0]} onChange={(e) => setFormData({...formData, endDate: e.target.value})} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm" />
                </div>
              </div>
            )}
                ...
                {numberOfDays > 1 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <p className="text-blue-800 text-sm font-medium">
                      📅 {numberOfDays}-day event detected from your dates. Add separate timelines for each day below.
                    </p>
                  </div>
                )}

                {/* Dynamic Day Timelines */}
                {Array.from({ length: numberOfDays }, (_, i) => i + 1).map(dayNum => {
                  const dayKey = `day${dayNum}`;
                  const dayTimeline = formData.timelines[dayKey] || [];
                  const colors = ['indigo', 'purple', 'teal', 'orange', 'pink'];
                    ...
                    <div key={dayKey} className="mb-6">
                      <div className="flex justify-between items-end mb-4">
                        <div>
                          <h3 className="text-lg font-bold text-slate-800">
                            {numberOfDays === 1 ? 'Event Timeline' : `Day ${dayNum} Timeline`}
                          </h3>
                          <p className="text-slate-500 text-sm">
                            {numberOfDays === 1 ? 'Schedule for the event.' : `Schedule for day ${dayNum} of the event.`}
                          </p>
                        </div>
                        <button
                          onClick={() => addTimelinePhase(dayKey)}
                          className={`px-4 py-2 bg-${color}-50 text-${color}-700 border border-${color}-100 rounded-lg text-sm font-medium hover:bg-${color}-100 transition-colors flex items-center gap-2`}
                          style={{ backgroundColor: `var(--${color}-50, #eef2ff)`, color: `var(--${color}-700, #4338ca)` }}
                        >
                      ...
                      {dayTimeline.length === 0 ? (
                        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-6 text-center">
                          <p className="text-slate-600 text-sm">No phases added. Click "Add Phase" to start.</p>
                        </div>
                      ) : (
                        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden shadow-sm">
                          ...
                          {dayTimeline.map((phase, index) => (
                            <div key={index} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 transition-colors">
                              <div className="col-span-3">
                                <input type="time" value={phase.startTime} onChange={(e) => updateTimelinePhase(index, 'startTime', e.target.value, dayKey)} className="w-full px-3 py-2 border border-slate-200 rounded bg-white text-sm" />
                              </div>
                              <div className="col-span-3">
                                <input type="time" value={phase.endTime} onChange={(e) => updateTimelinePhase(index, 'endTime', e.target.value, dayKey)} className="w-full px-3 py-2 border border-slate-200 rounded bg-white text-sm" />
                              </div>
                              <div className="col-span-5">
                                <input type="text" value={phase.phase} onChange={(e) => updateTimelinePhase(index, 'phase', e.target.value, dayKey)} placeholder="e.g. Opening Ceremony" className="w-full px-3 py-2 border border-slate-200 rounded bg-white text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" />
                              </div>
                              <div className="col-span-1 text-right">
                                <button onClick={() => removeTimelinePhase(index, dayKey)} className="text-slate-400 hover:text-red-500 transition-colors p-1">
          ...
          <div className="bg-slate-50 border-t border-slate-200 px-4 py-4 md:px-6 flex justify-end gap-3">
              <button onClick={saveAndTrain} disabled={isProcessing} className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700 shadow-sm transition-all flex items-center gap-2">
                 {isProcessing ? 'Saving & Training...' : '✓ Save & Train'}
              </button>
          </div>
        </div>
    </div>
