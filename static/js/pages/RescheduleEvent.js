const RescheduleEvent = () => {
  ...
  const fetchEventData = async (eventId) => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback: Try fetching from all events list
  ...
  const handleSave = async () => {
    ...
    // Validation: Past date check (Bug #9)
    const selectedDateTime = new Date(`${formData.date}T${formData.startTime}`);
    if (selectedDateTime < new Date()) {
      alert('Cannot schedule event in the past. Please select a future date.');
      return;
    }

    // Validation: End time after start time
    const startDateTime = new Date(`${formData.date}T${formData.startTime}`);
    const endDateTime = new Date(`${formData.endDate || formData.date}T${formData.endTime}`);
    ...
    try {
      ...
      const res = await fetch(`/api/events/${event.id}`, {
        ...
        body: JSON.stringify({
          ...
          additional_resources: event.additional_resources,
          organizing_department: event.organizing_department,

          // Updated fields (new values for reschedule)
    ...
    React.createElement('div', { className: 'max-w-6xl mx-auto' },
      ...
      React.createElement('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200' },
                ...
                React.createElement('div', { className: 'space-y-2' },
                  suggestedDates.map((suggestion, idx) =>
                    React.createElement('button', {
                      ...
                      className: `w-full px-3 py-2.5 border rounded-lg text-left transition flex flex-col group ${appliedSuggestion === idx
        ...
        React.createElement('div', { className: 'flex gap-3 p-4 border-t border-gray-200' },
          React.createElement('button', {
            onClick: () => window.location.hash = '#/admin',
            className: 'flex-1 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 transition text-sm'
          }, 'Cancel'),
          React.createElement('button', {
            onClick: handleSave,
            disabled: saving,
            className: 'flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm'
          },
